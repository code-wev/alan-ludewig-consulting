...
import { X, Info, ChevronDown, Upload, FileText, FileSpreadsheet, FileIcon, FileCode } from "lucide-react";
...
export function CustomRAModal({ isOpen, onClose }: CustomRAModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto no-scrollbar">
      <div 
        className="relative bg-white border-[1.5px] border-[#E3E6EC] rounded-xl w-full max-w-[846px] max-h-[90vh] overflow-y-auto p-5 flex flex-col gap-5 shadow-lg my-auto no-scrollbar"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between sticky top-0 bg-white z-10 pb-2">
          <h2 className="text-[20px] font-bold text-[#132651] font-sans">Request a Custom Template</h2>
          <button 
            onClick={onClose}
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-sm text-[#132651] font-sans">Template / Document Name</label>
              <input 
                type="text" 
                placeholder="e.g. Warehouse Fire Safety Checklist"
                className="w-full px-4 py-2 border-[1.5px] border-[#E3E6EC] rounded-md text-sm text-[#132651] placeholder:text-[#A3ACBA] outline-none focus:border-[#132651] transition-colors"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <select className="w-full px-4 py-2 border-[1.5px] border-[#E3E6EC] rounded-md text-sm text-[#132651] outline-none appearance-none bg-white focus:border-[#132651] transition-colors">
                    <option>Risk Assessment</option>
                  </select>
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#132651] pointer-events-none" />
                </div>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-sm text-[#132651] font-sans">Related Category</label>
                <div className="relative">
                  <select className="w-full px-4 py-2 border-[1.5px] border-[#E3E6EC] rounded-md text-sm text-[#132651] outline-none appearance-none bg-white focus:border-[#132651] transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm text-[#132651] font-sans">Project / Location</label>
                <div className="relative">
                  <select className="w-full px-4 py-2 border-[1.5px] border-[#E3E6EC] rounded-md text-sm text-[#132651] outline-none appearance-none bg-white focus:border-[#132651] transition-colors">
            ...
            {/* Request Details */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm text-[#132651] font-sans">Request Details</label>
              <textarea 
                placeholder="Describe the specific requirements or sections you need included..."
                className="w-full h-[60px] px-4 py-2 border-[1.5px] border-[#E3E6EC] rounded-md text-sm text-[#132651] placeholder:text-[#A3ACBA] outline-none resize-none focus:border-[#132651] transition-colors"
              />
            </div>

            {/* Upload Supporting File */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm text-[#132651] font-sans">Upload Supporting File</label>
              <div className="flex flex-col items-center justify-center p-4 gap-1 border-2 border-dashed border-[#E3E6EC] rounded-lg bg-slate-50/50 hover:bg-slate-50 cursor-pointer transition-colors">
                <Upload className="w-5 h-5 text-[#132651] mb-1" />
                <span className="text-sm font-bold text-[#132651] font-sans">Drag and drop your files here, or browse</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-sm text-[#132651] font-sans">Priority</label>
                <div className="relative">
                  <select className="w-full px-4 py-2 border-[1.5px] border-[#E3E6EC] rounded-md text-sm text-[#132651] outline-none appearance-none bg-white focus:border-[#132651] transition-colors">
              ...
              <div className="flex flex-col gap-1.5 justify-center">
                <label className="text-sm text-[#132651] font-sans mb-0.5">Preferred Output Format</label>
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" className="w-4 h-4 rounded border-[#D9DADC] text-[#1E3A8A] focus:ring-[#1E3A8A]" />
                    <span className="text-xs text-[#132651] font-sans">PDF</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" className="w-4 h-4 rounded border-[#D9DADC] text-[#1E3A8A] focus:ring-[#1E3A8A]" />
            ...
            <div className="flex flex-col gap-2 mt-1">
              <label className="flex items-center gap-2 cursor-pointer group">
                <div className="w-4 h-4 rounded border border-[#E3E6EC] flex items-center justify-center group-hover:border-[#132651] transition-colors bg-white">
                </div>
          ...
          <div className="sticky bottom-0 bg-white pt-2 pb-1">
            <Button className="w-full h-[44px] bg-[#132651] text-white font-bold hover:bg-[#132651]/90 rounded-lg text-sm">
              Submit Request
            </Button>
          </div>
        </div>
