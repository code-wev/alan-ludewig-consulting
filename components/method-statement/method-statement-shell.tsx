  ...
  return (
    <div className='flex flex-col gap-8 w-full mx-auto text-brand-primary'>
      ...
      <div className='flex flex-row p-4 gap-4 bg-[#E8F0FE] border border-[#ADC6FF]/30 rounded-lg w-full items-start'>
        ...
        <p className='text-[#132651] text-[13.5px] leading-[1.6] font-inter'>
        ...
        {METHOD_STATEMENT_STEPS.map((step, index) => {
          ...
          return (
            <div key={step.id} className='flex flex-col gap-2 min-w-[160px] flex-1 shrink-0'>
              <div
                className={`h-[6px] rounded-full w-full transition-all duration-300 ${
                  isBarActive ? 'bg-[#132651]' : 'bg-[#E3E6EC]'
                }`}
              />
              <span
                className={`text-[13.5px] font-semibold leading-[1.6] font-sans ${
                  isActive || isCompleted ? 'text-[#132651]' : 'text-[#5A6886]'
                }`}
              >
                {step.title}
              </span>
